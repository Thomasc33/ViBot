...
module.exports = {
    ...
    execute(message, args, bot) {
        ...
        const commandPanel = new Discord.EmbedBuilder()
            .setTitle('Commands')
            .setColor('#ff0000')
        if (args.length == 0) {
            const fields = {}
            for (const roleName in settings.roles) {
                if (!Object.hasOwn(settings.roles, roleName)) continue
                const roleID = settings.roles[roleName]
                if (isNaN(roleID) || !roleID) continue
                ...
                bot.commands.each(command => {
                    ...
                    const minimumRole = settings.commandsRolePermissions[command.name]
            ...
            for (const name of Object.keys(fields).sort((a, b) => fields[a].position - fields[b].position)) {
                if (fields[name].commands.length) {commandPanel.addFields({ name: name + '+', value: '```css\n' + fields[name].commands.join(' ') + '```' })}
            }
            ...
            if ((memberPosition < roleCache.get(settings.roles[command.role]).position && !override) || !settings.commands[command.name]) {
                ...
                } if (memberPosition < roleCache.get(settings.roles[settings.commandsRolePermissions[command.name]]).position) {
                    return message.channel.send('Command doesnt exist, check `commands` and try again')
                }
            }
            commandPanel.setTitle(command.name)
                .setColor('#ff0000')
