                    ...
                    for (const week of weeks) {
                        ...
                        currentWeekCol.on('collect', async i2 => {
                            ...
                            if (i2.customId == 'confirm') {
                                let currentWeekQuery = `UPDATE users SET ${week.currentWeekName} = `
                                switch (operator) {
                                    case 'a':
                                        currentWeekQuery += `${week.currentWeekName} + ${count} `
                                        break
                                    case 'r':
                                    default:
                                        currentWeekQuery += `${week.currentWeekName} - ${count} `
                                ...
                                currentWeekQuery += `WHERE id = '${member.id}'`
                                await db.promise().query(currentWeekQuery).then(() => send(i2), (err) => {
                                    console.log(err)
