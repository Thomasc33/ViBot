...
module.exports = {
    ...
    async execute(message, args, bot, db) {
        ...
        } if (vetRoles.length == 1) {
            // get the only vet role and assign it to the raider
            const vetRaiderRole = vetRoles[0]
        ...
        const choiceEmbed = new Discord.EmbedBuilder()
        ...
        await message.reply({ embeds: [choiceEmbed], ephemeral: true }).then(async confirmMessage => {
            ...
            const vetRaiderRole = vetRoles.find(vetRole => vetRole.name == choice)
    ...
    async addRole(bot, db, member, message, vetRaiderRole) {
        ...
        const confirmEmbed = new Discord.EmbedBuilder().setDescription(`${member} has been given ${vetRaiderRole}`)
