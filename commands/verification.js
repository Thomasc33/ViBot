...
module.exports = {
    ...
    async init(guild, bot, db) {
        ...
        if (!embedMessage) {
            const veriChannel = guild.channels.cache.get(settings.channels.verification)
            if (!veriChannel) return
            const messages = await veriChannel.messages.fetch({ limit: 1 })
            embedMessage = messages.first()
        }
        const reactionCollector = new Discord.ReactionCollector(embedMessage, { filter: (r, u) => !u.bot && r.emoji.name == '✅' })
    ...
    async verify(u, guild, bot, db) {
        ...
        if (!member) {member = await guild.members.fetch({ user: u.id, force: true })}

        const veriactive = guild.channels.cache.get(settings.channels.veriactive)
        const veripending = guild.channels.cache.get(settings.channels.manualverification)
        const verilog = guild.channels.cache.get(settings.channels.verificationlog)
        const veriattempts = guild.channels.cache.get(settings.channels.veriattempts)
        ...
        if (blInfo) {
            ...
            if (!blGuild || !bot.settings[blGuild.id]) {return u.send('You are currently blacklisted from verifying. Please DM me to contact mod-mail and find out why')}
        ...
        const activeMessage = await veriactive.send({ embeds: [LoggingEmbed] })

        // check other servers for verification
        const res = await this.reVerify(u, guild, bot, db) // true = verified, false = not verified
        ...
        const dms = await u.createDM()
        const embedMessage = await dms.send({ embeds: [embed] })
        ...
        // abort collector
        const abortCollector = new Discord.ReactionCollector(embedMessage, { filter: (r, u) => !u.bot && r.emoji.name == '❌' })
        ...
        abortCollector.on('collect', async (r, u) => {
            ...
            for (const i in reactionCollectors) {
        ...
        async function cancelVerification(reason, info) {
            ...
            switch (reason) {
                ...
                case 2:
                    ...
                    if (!blGuild) {embed.setDescription('You are currently blacklisted from verifying. Please DM me to contact mod-mail and find out why')} else {
                        ...
                        if (blGuild.id != guild.id) {LoggingEmbed.addFields([{ name: 'Staff?', value: currently ? '✅' : '❌', inline: true }])} else if (!currently) {embed.setDescription(`You are currently blacklisted from the __${blGuild.name}__ server and cannot verify. The person who blacklisted you is no longer staff. Please DM me and send mod-mail to that server to appeal.`)} else {embed.setDescription(`You are currently blacklisted from the __${blGuild.name}__ server and cannot verify. Please contact ${staff} in order to appeal. If they do not reply within 48 hours, feel free to mod-mail the ${blGuild.name} server to get assistance.`)}
                    }
                    break
                case 3:
        ...
        async function getIgn() {
            return new Promise(async (resolve, reject) => {
                const ignCollector = new Discord.MessageCollector(dms, { filter: m => !m.author.bot })
                ...
                ignCollector.on('collect', async m => {
                    if (m.content.split(/ +/).length > 1) {
                        ...
                        if (ign.replace(/[^a-z]/gi, '') != ign) {
                            ...
                            checkingIGN = true
                            const confirmReactionCollector = new Discord.ReactionCollector(embedMessage, { filter: (r, u) => !u.bot && (r.emoji.name == '✅' || r.emoji.name == '❌') })
        ...
        const dupes = guild.members.cache.filter(user => user.nickname).find(nick => nick.nickname.replace(/[^a-z|]/gi, '').toLowerCase().split('|').includes(ign.toLowerCase()))
        if (dupes) {
        ...
        const vericode = `VIBOT-${Math.floor(Math.random() * 10000)}`
        ...
        embedMessage.edit({ embeds: [embed] })
        const veriCodeReactionCollector = new Discord.ReactionCollector(embedMessage, { filter: (r, u) => !u.bot && r.emoji.name == '✅' })
        ...
        veriCodeReactionCollector.on('collect', async (r, u) => {
            ...
            const userInfo = await realmEyeScrape.getGraveyardSummary(ign).catch(er => {
            ...
            if (!found) {
                ...
                const denyReason = []
                // stars
                if (parseInt(userInfo.rank) < settings.autoveri.stars) {
                    denyReason.push({
                        reason: 'Star Count',
                ...
                // fame
                if (parseInt(userInfo.fame) < settings.autoveri.fame) {
                    denyReason.push({
                        reason: 'Fame Count',
                ...
                // discord account age
                if (u.createdTimestamp > Date.now() - (settings.autoveri.discordage * 2592000000)) {
                    denyReason.push({
                        reason: 'Discord Account Age'
                    })
                }
                // realm account age
                const days = userInfo.created.split(/ +/)
                let daysValid = false
                for (const i in days) { if (parseInt(days[i].replace(/[^0-9]/g, '')) >= settings.autoveri.realmage) daysValid = true }
                if (!(userInfo.created.includes('year') || daysValid)) {
                    denyReason.push({
                        reason: 'Realm Account Age'
                    })
                }
                // death count
                if (parseInt(userInfo.deaths[userInfo.deaths.length - 1]) < settings.autoveri.deathcount) {
                    denyReason.push({
                        reason: 'Death Count',
        ...
        async function manualVerify(reasons, data) {
            ...
            const manualEmbed = new Discord.EmbedBuilder()
            ...
            const m = await veripending.send({ embeds: [manualEmbed] })
            await m.react('🔑')
    ...
    async manualVerifyUpdate(guild, bot, db) {
        const settings = bot.settings[guild.id]
        const veriPending = guild.channels.cache.get(settings.channels.manualverification)
        const messages = await veriPending.messages.fetch({ limit: 100 })
    ...
    async watchMessage(message, bot, db) {
        ...
        const member = await message.guild.members.fetch(member_id).catch(e => ErrorLogger.log(e, bot))
        // variables
        ...
        const desc = embed.data.author.name.split(/ +/)
        const ign = desc[desc.length - 1]
        // start key reaction collector
        if (!message.reactions.cache.has('🔑')) message.react('🔑')
        const reactionCollector = new Discord.ReactionCollector(message, { filter: (r, u) => !u.bot && r.emoji.name == '🔑' })
        reactionCollector.on('collect', async (r, u) => {
            ...
            const reactor = message.guild.members.cache.get(u.id)
            // stop old reaction collector, start new reaction collector
            reactionCollector.stop()
            const checkXCollector = new Discord.ReactionCollector(message, { filter: (r, u) => u.id == reactor.id && (r.emoji.name === '✅' || r.emoji.name === '❌' || r.emoji.name === '🔒') })
            ...
            checkXCollector.on('collect', async (r, u) => {
                ...
                if (r.emoji.name === '✅') {
                    ...
                    // log in veri-log
                    const veriEmbed = new Discord.EmbedBuilder()
                        .setColor('#00ff00')
                ...
                // x
                    ...
                    const reasonCollector = new Discord.ReactionCollector(message, { filter: (r, u) => u.id == reactor.id && (r.emoji.name === '1️⃣' || r.emoji.name === '2️⃣' || r.emoji.name === '3️⃣' || r.emoji.name === '4️⃣' || r.emoji.name === '🔒') })
                    // add reacts
                    message.react('1️⃣')
    ...
    async reVerify(u, guild, bot, db) {
        return new Promise(async (res, rej) => {
            ...
            const uniqueNames = [...new Set(nicks)]
            ...
            const m = await u.send({ embeds: [embed] })
            ...
            reactionCollector.on('collect', async (r, u) => {
                switch (r.emoji.name) {
                    ...
                    default:
                        const retryMessage = await message.channel.send('There was an issue with the reaction. Please try again')
