...
module.exports = {
    ...
    async execute(message, args, bot, db){
        ...
        db.query(`SELECT * FROM users WHERE id = '${member.id}'`, (err, rows) => {
            ...
            message.guild.channels.cache.find(c => c.name === 'vial-logs').send(`Vial added to ${member} (${member.nickname}), logged by ${message.member} (${parseInt(rows[0].vialUsed) + 1} total pops)`)
        })
    }
}