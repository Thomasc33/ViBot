...
async function resetPoints(points_log_message, message, bot, db) {
    ...
    pointsLog = []
    for (const point_string of points_array) {
        point_string_split = point_string.split('`')
        const points = Number(point_string_split[1])
        const user = point_string_split[0].slice(3, point_string_split[0].indexOf('>'))
    ...
    const throwaway_embed = new Discord.EmbedBuilder()
...
async function resetPointsUsers(points_users, early_location_cost, message, db) {
    let sum = 0
    for (const user of points_users) {
        try {
...
module.exports = {
    ...
    async execute(message, args, bot, db) {
        ...
        await run_logs.messages.fetch()
            .then(messages => {
                const history_embeds = messages.filter(m => m.id === history_id)
                if (history_embeds.size != 0) {
                    const history_embed_title = history_embeds.first().embeds[0].title.split(' ').pop()
        ...
        if (history_message_fields[6].value != 'None!') {
            ...
            early_location_cost = Number(history_message_fields[6].name.split(' ').pop())
        }
        ...
        await point_channel.messages.fetch()
            .then(messages => {
                const points_log_embeds = messages.filter(m => m.id === points_mid)
