...
module.exports = {
    ...
    async execute(message, args, bot) {
        ...
        const argList = prefix + 'changename ' + (staff ? this.args : '<new name>')
        try {
            if (!staff && args.length != 1) throw new Error(`Invalid arguments: ${argList}`)
            if (args.length == 1) {await this.change(message, message.member, args.shift(), args, bot, settings, staff)} else {
                const memberArg = args.shift(),
                    memberId = memberArg.replace(/[^0-9]/gi, '')
                const member = message.guild.members.cache.get(memberId)
                if (!member) throw new Error(`Could not find member ${memberArg} (id: ${memberId})`)
    ...
    async change(message, member, altName, args, bot, settings, staff) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (!altName) {
                reject(new Error('An IGN was not provided.'))
                return
            }
            if (!staff && altName.length > 15) {
                reject(new Error('An IGN cannot be more than 15 characters under normal circumstances. If you are an exception, please contact a Security+ staff member.'))
                return
            }
            ...
            for (let j = 0; j < names.length; j++) {
                if (names[j] == altName) { // their name matches exactly
                    reject(new Error((own ? 'You already have' : `${member} already has`) + ` the nickname ${altName}!`))
                }
                if (names[j].toLowerCase() == altName.toLowerCase()) { // They have a case insensitive name match, request adjust name
                    idx = j
                    changeName(message, bot, settings, member, names, idx, altName, userPrefix)
            ...
            let image
            if (staff) {
                if (message.attachments.size) image = await message.attachments.first().proxyURL // attached image
                else if (args.length) image = args.shift() // image as last argument

                if (image && !ext.isHttpURL(image)) { // image provided but it was not good
                    reject(new Error('There was an error retrieving the image.'))
                    return
                }
            }
            ...
            if (!data || !data.historyName) {
                ...
                if (!staff || !image) {
                    reject(new Error((own ? 'Your' : `${member}'s`)
            ...
            if (data) {
                if (data.altName) {altName = data.altName}
                if (typeof data.idx === 'number') {idx = data.idx}
                if (data.memberPrefix) {userPrefix = data.memberPrefix}
            }
            // new name length greater than 32?
            if ([altName, ...names.slice(0, idx), ...names.slice(idx + 1)].join(' | ').length > 32) {
                reject(new Error(`Changing ${own ? 'your' : '<@!' + member.id + ">'s"} name from \`${names[idx]}\` to \`${userPrefix}${altName}\` will exceed the maximum nickname length of 32. Please${own ? ' ' : ' ask staff to '}remove an alt before proceeding.`))
                return
            }
            ...
            if (dupe) {
                if (!staff) {
                    reject(new Error(`There is already a user with the IGN ${altName} in the server. If this is in fact your name, please contact a Security+ staff member to resolve the issue immediately.`))
                    return
                }
            ...
            changeName(message, bot, settings, member, names, idx, altName, userPrefix, image, historyName, history)
                .then(result => resolve(result))
                .catch(err => reject(err))
        })
    }
...
const changeName = async (message, bot, settings, member, names, idx, altName, userPrefix, image, historyName, history) => new Promise(async (resolve, reject) => {
    ...
    try {
        if (await confirm.confirmButton(message.author.id)) {
            ...
            try {
                ...
                reject(new Error(`There was an issue changing ${member}'s nickname: \`${err.toString().split('\n')[0]}\``))
            ...
            if (image) embed.setImage(image) // image was provided
        ...
        reject(new Error(`There was an error changing ${member}'s name.`))
        ErrorLogger.log(err, bot, message.guild)
    }
})
