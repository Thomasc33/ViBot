...
module.exports = {
    ...
    async createMessage(message, bot, db) {
        ...
        const vetVeriChannel = message.guild.channels.cache.get(settings.channels.vetverification)
        if (!vetVeriChannel) return message.channel.send('Vet Verification channel not found')
        const vetVeriEmbed = new Discord.EmbedBuilder()
    ...
    async init(guild, bott, db) {
        bot = bott
        const settings = bott.settings[guild.id]
        if (!embedMessage) {
            const vetVeriChannel = guild.channels.cache.get(settings.channels.vetverification)
            if (vetVeriChannel == null) return
            const messages = await vetVeriChannel.messages.fetch({ limit: 1 })
            embedMessage = messages.first()
        }
        const reactionCollector = new Discord.ReactionCollector(embedMessage, { filter: checkFilter })
    ...
    async vetVerify(u, guild, db) {
        ...
        const vetRaider = settings.roles.vetraider
        const veriLog = guild.channels.cache.get(settings.channels.verificationlog)
        const veriPending = guild.channels.cache.get(settings.channels.manualvetverification)
        const ign = member.nickname.replace(/[^a-z|]/gi, '').toLowerCase().split('|')[0]
        ...
        if (!dungeon.webdata) {
            db.query(`SELECT * FROM users WHERE id = '${u.id}'`, (err, rows) => {
                ...
                if (dungeon.dbnames) for (const i of dungeon.dbnames) if (rows[0][i]) loggedRuns += parseInt(rows[0][i])
        ...
        const userInfo = await realmEyeScrape.getUserInfo(ign)
        let maxedChars = 0
        let meleeMaxed = 0
        let realmEyeRuns = 0
        if (settings.backend.realmeyestats) {
            for (const i in userInfo.characters) {
                const char = userInfo.characters[i]
                if (char.stats == '8/8') {
                    maxedChars += 1
        ...
        if (dungeon.realmeyestring) {
            const graveyard = await realmEyeScrape.getGraveyardSummary(ign).catch(er => null)
            if (graveyard) {
                for (const i in graveyard.dungeonCompletes) {
                    const achievement = graveyard.dungeonCompletes[i]
        ...
        if (dungeon.exaltStats.length > 0) {
            ...
            for (const i of dungeon.exaltStats) {
                for (const j in exaltStats.exaltations) {
                    const e = exaltStats.exaltations[j]

                    if (e[i]) {
                        e[i] = parseInt(e[i].toLowerCase().replace('+', ''))
                        if (['health', 'mana'].includes(e[i])) {e[i] = e[i] / 5}
                        switch (e[i]) {
        ...
        if (problems.length == 0) {
            ...
            if (dungeon.lootInfo && lootInfo[dungeon.lootInfo]) {
                for (const i in userInfo.characters) {
                    const char = userInfo.characters[i]
                    // weapon
                    if (char.weapon) {
            ...
            if (dungeon.webAppLink) {description += ` - [Web App](https://losthalls.org/profile/${ign})`}
            ...
            if (!profile) {mainEmbed.addFields([{ name: 'Bot-Logged Runs:', value: `${loggedRuns || 0}` }])} else {
            ...
            for (const i of dungeon.exaltStats) {
                ...
                const ishpmana = ['health', 'mana'].includes(i.toLowerCase())
                for (const j in exaltStats.exaltations) {
            ...
            const pendingMessage = await veriPending.send({ embeds: [mainEmbed] })
    ...
    async pendingModule(message, db) {
        const { manualVetVerifyLog } = this
        const settings = bot.settings[message.guild.id]
        if (watching.includes(message.embeds[0].footer.text)) return
        watching.push(message.embeds[0].footer.text)
        if (!message.reactions.cache.has('🔑')) message.react('🔑')
        const vetRaider = message.guild.roles.cache.get(settings.roles.vetraider)
        const keyCollector = new Discord.ReactionCollector(message, { filter: KeyFilter })
        keyCollector.on('collect', async (r, u) => {
            const reactor = message.guild.members.cache.get(u.id)
            message.reactions.removeAll()
                .then(message.react('💯'))
                .then(message.react('👋'))
                .then(message.react('🔒'))
            const ManualVerificationCollector = new Discord.ReactionCollector(message, { filter: ManualFilter })
            ManualVerificationCollector.on('collect', async (r, u) => {
                if (!(u.id == reactor.id)) return
                const embed = new Discord.EmbedBuilder()
                embed.data = message.embeds[0].data
                const member = message.guild.members.cache.get(embed.data.footer.text)
                const info = {
                    role: vetRaider,
                    guild: message.guild,
                    member,
                ...
                function removeFromArray(id) {
                    const index = watching.indexOf(id)
                    if (index > -1) {
