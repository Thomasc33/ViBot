...
module.exports = {
    ...
    execute(message, args, bot) {
        ...
        if (message.channel.parent.name.toLowerCase() === settings.categories.raiding) {
            ...
            let channel = message.guild.channels.cache.find(c => c.type == Discord.ChannelType.GuildCategory && c.name == 'Events').children.find(c => c.name.includes(args[0]) && !c.name.includes('Realm Clearing'))
