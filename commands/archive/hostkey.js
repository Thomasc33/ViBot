...
module.exports = {
    ...
    async execute(message, args, bot) {
        ...
        const panel_embed = new Discord.EmbedBuilder()
            .setAuthor({ name: "Select keys to pop" })
            .setColor("#3498db")
        ...
        panel.collector.on('end', reason => {
            if (reason != "Manual") {
                ...
                panel_embed.setAuthor({ name: `Panel Timed Out` })
        ...
        panel.collector.on('collect', async (reaction, user) => {
            ...
            switch (reaction.emoji.name) {
                case '❌':
                    ...
                    this.activePanels = this.activePanels.filter(id => id != message.author.id);
                    panel_embed.setAuthor({ name: `Cancelled Notifications` })
                ...
                case '✅':
                    ...
                    panel_embed.setAuthor({ name: `Notification Sent` })
                ...
                case '🔑':
                    const embed = new Discord.EmbedBuilder()
                        .setAuthor({ name: "Select a Dungeon" })
                    ...
                    if (reply) {
                        ...
                        panel_embed.data.fields = fields;
                        panel.edit({ embeds: [panel_embed] });
                    }
                    return;
            }
