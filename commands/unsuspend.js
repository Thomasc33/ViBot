    ...
    execute(message, args, bot, db) {
        ...
        db.query(`SELECT * FROM suspensions WHERE id = '${member.id}' AND suspended = true`, async (err, rows) => {
            ...
            if (rows && rows.length == 0) {
                ...
                try {
                    for (const i in settings.lists.discordRoles) {
                    ...
                    try {
                        const embed = Discord.EmbedBuilder.from(bot.guilds.cache.get(guildId).channels.cache.get(settings.channels.suspendlog).messages.cache.get(proofLogID).embeds.shift())
                        ...
                        const messages = await bot.guilds.cache.get(guildId).channels.cache.get(settings.channels.suspendlog).messages.fetch({ limit: 100 })
        ...
        function unPerma() {
            ...
            db.query(`SELECT * FROM suspensions WHERE perma = true AND suspended = true AND id = '${member.id}'`, async (err, rows) => {
                if (rows.length == 0) {
                    const confirm = await message.channel.send(`I do not have any records of ${member} being perma suspended. Would you like to remove suspended, and add raider back?`)
                    confirm.react('✅')
                        .then(confirm.react('❌'))
                    const reactionCollector = new Discord.ReactionCollector(confirm, { filter: (r, u) => u.id = message.author.id && (r.emoji.name == '❌' || r.emoji.name == '✅'), time: 60000 })
