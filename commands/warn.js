...
module.exports = {
    ...
    async execute(message, args, bot, db) {
        ...
        let lowest_staff_role = guildSettings[message.guild.id].roles["eventrl"];
        if(lowest_staff_role) {
                ...
                if(member_is_staff >= 0) {
                    ...
                    let high_supreme_roles = message.member.roles.cache.filter(role => role.id == guildSettings[message.guild.id].roles["moderator"] ||
                                                                                 role.id == guildSettings[message.guild.id].roles["headrl"] ||
                                                                                 role.id == guildSettings[message.guild.id].roles["headeventrl"] ||
                                                                                 role.id == guildSettings[message.guild.id].roles["officer"]);
                    if(high_supreme_roles.size == 0) {
                        //warner doesnt have high supreme role and member is staff, shouldn't be able to warn
                        message.channel.send("Could not warn that user as they are staff and your highest role isn't high enough. :^]");
                        return;
                    }
