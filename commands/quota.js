...
module.exports = {
    ...
    async execute(message, args, bot, db) {
        ...
        if (!args.length) {
            for (const quota of quotaList) this.sendEmbed(message.channel, db, bot, false, guildQuotas, quota)
            return
        }
    ...
    async fullReset(guild, db, bot, quotaList) {
        ...
        if (ignore) await db.promise().query(`DELETE FROM ignoreCurrentWeek WHERE guildId = ${guild.id}`)
    },
    async newWeek(guild, bot, db, settings, guildQuotas, quota) {
        ...
        if (rolling.length) {
            const ignore = await excuses.getIgnore(guild.id, db)
            if (ignore) await db.promise().query(`UPDATE users SET ${rolling[0].column} = 0`)
            else {
                const rlist = quota.roles.map((role, i) => ({ role: guild.roles.cache.get(settings.roles[role]), req: quota.quota[i], rollmax: quota.rollmax[i] })).sort((a, b) => b.position - a.position)
                const membersUpdated = {}
                for (const { role, req, rollmax }
                    of rlist) {
                    const ids = []
                    role.members.forEach(member => {
                        if (membersUpdated[member.id]) return
                        membersUpdated[member.id] = true
                        ids.push(`'${member.id}'`)
                    })
                    if (ids.length) {
        ...
        await db.promise().query(`UPDATE users SET ${quota.values.filter(v => !v.rolling).map(v => `${v.column} = 0`).join(', ')}`)
        await this.update(guild, db, bot, settings, guildQuotas, quota)
    },
    async update(guild, db, bot, settings, guildQuotas, quota) {
        const currentweek = guild.channels.cache.get(settings.channels[quota.currentweek])
    ...
    sendEmbed(channel, db, bot, nw, guildQuotas, quota) {
        ...
        return new Promise((resolve, reject) => {
            const emojiList = quota.values.map(value => value.emoji ? `${value.emoji}: **${value.name}**` : '')
            for (const i in emojiList) { if (emojiList[i] == '') { emojiList.splice(i, 1) } }
            const embed = new Discord.EmbedBuilder()
            ...
            db.query('SELECT id, ' + quota.values.map(v => v.column).join(', ') + `, ${combine}, ${unrolled} FROM users WHERE ` + quota.values.map(v => `${v.column} != 0`).join(' OR ') + ' order by total desc', async (err, rows) => {
                ...
                for (const user of rows) {
                    ...
                    if (!member || !member.roles.cache.some(r => roles.includes(r.id)) || member.roles.cache.some(r => ignore.includes(r.id))) continue
                    csvData += `${user.id},${member?.nickname},${user.unrolled},${user.total}\n`
                    const result = `\`${position.toString().padStart(3, ' ')}.\` <@!${user.id}>\n**Points** \`${user.total}\` (`
                ...
                if (channel.id == settings.channels[quota.currentweek]) {
                    ...
                    try {
                        if (CachedMessages[guild.id][quota.id] && CachedMessages[guild.id][quota.id].length > 0) {
                            if (embeds.length == CachedMessages[guild.id][quota.id].length) await editMessages()
                            else resendMessages()
                        ...
                        async function resendMessages() {
                            ...
                            for (const embed of embeds) {
                                CachedMessages[guild.id][quota.id].shift(await channel.send({ embeds: [embed] }))
                            }
                        }
                        async function gatherMessages() {
                            ...
                            if (messages.size == embeds.length) {
                                messages.forEach(m => CachedMessages[guild.id][quota.id].push(m))
                                await editMessages()
                            } else resendMessages()
                        }
                        async function editMessages() {
                            for (const i of CachedMessages[guild.id][quota.id]) {
                                if (embeds.length == 0) {
                                    i.delete()
                                    delete CachedMessages[guild.id][quota.id][i]
                                }
                                await i.edit({ embeds: [embeds.pop()] })
                            }
                        }
                    } catch (er) { console.log(er) }
                } else for (const embed of embeds) channel.send({ embeds: [embed] })
                if (nw) await channel.send({ files: [new Discord.AttachmentBuilder(Buffer.from(csvData, 'utf-8'), { name: 'currentweekResetData.csv' })] })
                resolve(true)
            })
        })
    }
...
function getNextReset(reset) {
    switch (reset) {
        case 'biweekly':
            if (!(moment().diff(moment(1413378000), 'week') % 2)) return moment().add(1, 'week').startOf('week').unix()
