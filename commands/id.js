module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!member) member = message.guild.members.cache.filter(user => user.nickname).find(nick => nick.nickname.toLowerCase() == memberSearch || nick.nickname.replace(/[^a-z|]/gi, '').toLowerCase().split('|').includes(memberSearch.toLowerCase()))
