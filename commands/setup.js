...
const roles = [
    ...
    'moonlightping',
    'eventBoiPing',
    'advancedSteamworksPing',
    'advancedNestPing',
    'almostShattersBanner',
    'almostMoonlightBanner',
    'almostOryxBanner',
    'oryxBanner',
    'veteranOryxBanner',
    'almostHallsBanner',
...
const channels = ['modmail',
    'verification',
    'manualverification',
    'vetverification',
    'manualvetverification',
    'verificationlog',
    'accursedverification',
    'modlogs',
    'history',
    'suspendlog',
    'rlfeedback',
    ...
    'activitylog',
    'raidingrules',
    'forwardedModmailMessage',
    'fameLeaderCurrentWeek',
    'fameLeaderPastWeeks',
    'serverLeaveChannel',
    'botCommands',
    'raidingBotCommands',
    ...
    'adminBotCommands',
    'officerBotCommands',
    'headRaidLeaderBotCommands',
    'securityBotCommands']
const raiding = ['category1',
    'templateChannel1',
    'partneredStatusChannel1',
    'statusChannel1',
    'commandsChannel1',
    'activeChannel1',
    'category2',
    'templateChannel2',
    'partneredStatusChannel2',
    'statusChannel2',
    'commandsChannel2',
    'activeChannel2',
    'category3',
    'templateChannel3',
    'partneredStatusChannel3',
    'statusChannel3',
    'commandsChannel3',
    'activeChannel3',
    'category4',
    'templateChannel4',
    'partneredStatusChannel4',
    'statusChannel4',
    'commandsChannel4',
    'activeChannel4',
    'category5',
    'templateChannel5',
    'partneredStatusChannel5',
    'statusChannel5',
    'commandsChannel5',
    'activeChannel5',
    'category6',
    'templateChannel6',
    'partneredStatusChannel6',
    'statusChannel6',
    'commandsChannel6',
    'activeChannel6',
    'category7',
    'templateChannel7',
    'partneredStatusChannel7',
    'statusChannel7',
    'commandsChannel7',
    'activeChannel7',
    'category8',
    'templateChannel8',
    'partneredStatusChannel8',
    'statusChannel8',
    'commandsChannel8',
    'activeChannel8',
    'category9',
    'templateChannel9',
    'partneredStatusChannel9',
    'statusChannel9',
    'commandsChannel9',
    'activeChannel9',
    'category10',
    'templateChannel10',
    'partneredStatusChannel10',
    'statusChannel10',
    'commandsChannel10',
    'activeChannel10']
const voice = ['lounge', 'vetlounge', 'eventlounge', 'afk']
const voiceprefixes = ['raidingprefix', 'vetprefix']
const backend = ['modmail',
    'verification',
    'vetverification',
    'points',
    'supporter',
    ...
    'automod',
    'characterparse',
    'forwadedMessageThumbsUpAndDownReactions',
    'giveeventroleonverification',
    'upgradedCheck',
    'sendmissedquota',
    'onlyUpperStaffSuspendStaff',
    'giveEventRoleOnDenial2',
    'useUnverifiedRole',
    'punishmentsWarnings',
    'punishmentsSuspensions',
    'punishmentsMutes',
    'allowAdditionalCompletes',
    'miniBossGuessing',
    'logServerLeave',
    'isLogAssistsCapped']
const numerical = ['ticketlimit',
    'supporterlimit',
    'keyalertsage',
    'waitnewkeyalert',
    'prunerushersoffset',
...
const supporter = ['supporterCooldownSeconds1',
    'supporterCooldownSeconds2',
    'supporterCooldownSeconds3',
    'supporterCooldownSeconds4',
    'supporterCooldownSeconds5',
    'supporterCooldownSeconds6',
    'supporterUses1',
    'supporterUses2',
    'supporterUses3',
    'supporterUses4',
...
let commands = []
let commandsRolePermissions = []

const checkPanels = ['duplicateNicknames',
    'verifiedWithoutNickname',
    'unverifiedWithNickname',
    'removeRolesFromUserWithRole',
    'userWithTwoRoles',
    'addRolesToUsersWithRoles',
    'userWithAtleastOneOf',
    'openModmails',
    'openVerifications',
    'openVeteranVerifications',
    'falseSuspensions',
    'buttonGuide',
    'buttonAutoFix']
...
module.exports = {
    ...
    async execute(message, args, bot, db) {
        ...
        if (args.length == 0) {
            ...
            const mainMenu = new Discord.MessageCollector(message.channel, { filter: m => m.author.id == message.author.id })
            mainMenu.on('collect', async m => {
                ...
                async function menu(array, arrayName, type) {
                    ...
                    const menuCollector = new Discord.MessageCollector(message.channel, { filter: m => m.author.id == message.author.id })
                    menuCollector.on('collect', async m => {
                        ...
                        if (m.content.replace(/[^0-9]/g, '') != m.content) {
                            ...
                            const menuMessageCollector = new Discord.MessageCollector(message.channel, { filter: m => m.author.id == message.author.id })
                            ...
                            // Get array value
                            const cur = bot.settings[message.guild.id][arrayName][[array[parseInt(num) - 1]]]
                            ...
                            const str = cur.map((v, i) => `${parseInt(i) + 1}: ${v}`).join('\n')
                            ...
                            collector.on('collect', async m => {
                                ...
                                if (m.content.toLowerCase() == 'add' || m.content.toLowerCase() == 'new') {
                                    ...
                                    if (isNaN(parseInt(m.content))) { const m = await message.channel.send('Response not recognized. Try again.'); setTimeout(() => m.delete(), 5000); return }

                                    // Proceed with Deletion
                                    collector.stop()
                                    const selectionIndex = parseInt(m.content) - 1

                                    // Ensure selection is in range
                                    const selection = cur[selectionIndex]
    ...
    autoSetup(guild, bot) {
        ...
        for (const i in roles) {
            ...
            if (!bot.settings[guild.id].roles[role]) {
                const r = guild.roles.cache.find(r => r.name === getDefaultRoleName(role))
        ...
        for (const i in channels) {
            ...
            if (!bot.settings[guild.id].channels[channel]) {
                const c = guild.channels.cache.find(c => c.name === getDefaultChannelName(channel) && c.type == Discord.ChannelType.GuildText)
        ...
        for (const i in voice) {
            ...
            if (!bot.settings[guild.id].voice[v]) {
                const vc = guild.channels.cache.find(c => c.name === getDefaultVoiceName(v) && c.type == Discord.ChannelType.GuildVoice)
        ...
        for (const i in vetverireqs) {
            const r = vetverireqs[i]
        ...
        for (const key in bot.settings[guild.id].points) {
            if (!points.includes(key)) { delete bot.settings[guild.id].points[key] }
        }
        for (const i of commands) {
            if (!bot.settings[guild.id].commands[i] && bot.settings[guild.id].commands[i] !== false) bot.settings[guild.id].commands[i] = !bot.commands.get(i).guildSpecific // this might not work lol
        }
        for (const key in bot.settings[guild.id].commands) {
            if (!commands.includes(key)) { delete bot.settings[guild.id].commands[key] }
        }
        for (const i of commandsRolePermissions) {
        ...
        for (const key in bot.settings[guild.id].commandsRolePermissions) {
        ...
        for (const i in quotapoints) {
            const n = quotapoints[i]
        ...
        for (const i of checkUserExceptions) {
        ...
        for (const i of checkRoleExceptions) {
        ...
        for (const i of removeRoleFromUserWithRoles) {
        ...
        for (const i of addRolesToUsersWithRoles) {
