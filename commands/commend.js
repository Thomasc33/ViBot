...
module.exports = {
    ...
    async execute(message, args, bot, db) {
        ...
        const validRoles = settings.lists.commendRoles
        ...
        const roleToAdd = message.guild.findRole(settings.roles[args[1]])
        ...
        const minimumStaffRole = message.guild.findRole(settings.roles.minimumStaffRole)
        ...
        await message.channel.send(`Are you sure you want to give ${roleToAdd.name} to ${member.displayName}?`).then(async viMessage => {
            if (await viMessage.confirmButton(message.author.id)) {
                const modlog = viMessage.guild.channels.cache.get(settings.channels.modlogs)
                if (modlog) {
                    await modlog.send({
                        embeds: [
