...
module.exports = {
    ...
    execute(message, args, bot, db) {
        ...
        if (args.length == 0) {
            ...
            console.log(popInfo)
            const keyCountEmbed = new Discord.EmbedBuilder().setDescription(`**The ${message.guild.name} server has the following key roles configured:**\n\n`).setColor('#ff0000')
            if (!popInfo || !popInfo.length) keyCountEmbed.data.description += 'No key roles have been setup in this Discord server.'
                ...
                if (!found) {keyCountEmbed.data.description += 'No key roles have been setup in this Discord server.'}
            }
    ...
    async checkAll(guild, bot, db) {
        ...
        db.query(`SELECT id, ${rows} FROM users WHERE ${wheres}`, (err, rows) => {
            ...
            for (const i in rows) {checkRow(guild, bot, rows[i])}
        })
    },
        ...
        db.query(`SELECT id, ${rows} FROM users WHERE ${wheres}`, (err, rows) => {
            ...
            guild.members.fetch().then(members => {
                for (const [id, member] of members) {
                    const toRemove = [],
                        toAdd = []
                    if (!dbmembers[id]) {dbmembers[id] = {}}
                    for (const keyInfo of popInfo) {
                        if (!settings.roles[keyInfo.role]) continue
                        let count = 0
                        for (const [keyType, keyName] of keyInfo.types) {count += dbmembers[id][keyType] || 0}
                        if (count >= keyInfo.amount && !member.roles.cache.has(settings.roles[keyInfo.role])) {toAdd.push(settings.roles[keyInfo.role])} else if (count < keyInfo.amount && member.roles.cache.has(settings.roles[keyInfo.role])) {toRemove.push(settings.roles[keyInfo.role])}
                    }
                    ...
                    if (changes[id].add.length == 0 && changes[member.id].remove.length == 0) {delete changes[member.id]}
                }
...
function checkRow(guild, bot, row, member) {
    return new Promise(async (res) => {
        ...
        for (const keyInfo of popInfo) {
            if (!settings.roles[keyInfo.role]) continue
            let count = 0
            for (const [keyType, keyName] of keyInfo.types) {count += row[keyType] || 0}
