...
function log(bot) {
    if (!botSettings.rateLimitLogger?.discord) return
...
function processLimit(bot, limit) {
    // During testing some of the stack traces were just short of the useful information
    // 16 should be enough over the default of 10 to catch most real locations, barring async shenanigans
    ...
    if (botSettings.rateLimitLogger?.console) {
        console.log(textPrint(data))
    }

    if (botSettings.rateLimitLogger?.discord) limits.push(data)
}
...
function rateLimitLogger(bot) {
    if (!botSettings.rateLimitLogger?.console && !botSettings.rateLimitLogger?.discord) return

    bot.rest.on('restDebug', data => {
        if (!data.includes('Major parameter')) {
            if (botSettings.rateLimitLogger?.console) console.log(data)
            return
        }
