...
class ProxyHandler {
    constructor() {
        ...
        this.user = 'esracdjj';
        this.pass = 'tvcpn53ga6ky';
        this.proxyList = [];
        this.position = 0;
    }

    get authorization() {
        return 'Token 109d8619d4895cae2efa8993aa2fc39bcf3a7ee7';
    }
    ...
    async updateProxyList() {
        await new Promise((res, rej) => {
            const options = {
                url: this.api,
                headers: { 'Authorization': this.authorization, 'X-Webshare-SubUser': '17787' }
            }
            ...
            request(options, async (err, resp, html) => {
                ...
                for (let i of body.results) {
                    this.proxyList.push(new ProxyURL(this.user, this.pass, i.proxy_address, i.ports.http, this, await this.getCert()));
                }
                res();
            })
        });
