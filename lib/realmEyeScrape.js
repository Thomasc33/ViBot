...
if (settings.openSslLocation) pem.config({ pathOpenSSL: settings.openSslLocation })
...
class ProxyHandler {
    ...
    async next(noConfirm) {
        if (!this.proxyList.length) {await this.updateProxyList()}

        this.position++
        if (this.position >= this.proxyList.length) {this.position = 0}

        let proxy = this.proxyList[this.position]

        while (!noConfirm && this.proxyList.length && !await proxy.confirm()) {
            if (++proxy.failed >= 3) {this.proxyList.splice(this.position, 1)} else {this.position++}
        ...
        return new Promise(async (resolve, reject) => {
            ...
            handler.query({ url: `https://www.realmeye.com/player/${ign}` }).then(async ({ html }) => {
                ...
                for (var i = 0; i < rows.length; i++) {
                    try {
                        const row = rows[i]
                        const title = $(row).children('td:nth-child(1)').text()
                        const text = $(row).children('td:nth-child(2)').text()
                        switch (title) {
                            case 'Characters':
                                var chars = text
                            case 'Skins':
                ...
                const desc = [$('.line1.description-line').text(), $('.line2.description-line').text(), $('.line3.description-line').text()]
                const characters = []
                rows = $('.table.table-striped.tablesorter > tbody').find('tr')

                // list of all items
                const items = $('span .item')

                // list of all character stats
                const charStats = $('span.player-stats')
                ...
                for (var i = 0; i < rows.length; i++) {
                    try {
                        ...
                        if (test && test.name === 'a') {idx++}

                        // each weapon span starts at an index thats modulo 4
                        const itemStartIndex = i * 4
                        const weapon = $(items[itemStartIndex]).attr('title')
                        const ability = $(items[itemStartIndex + 1]).attr('title')
                        const armor = $(items[itemStartIndex + 2]).attr('title')
                        const ring = $(items[itemStartIndex + 3]).attr('title')
        ...
        return new Promise(async (resolve, reject) => {
            handler.query(`https://www.realmeye.com/graveyard-summary-of-player/${ign}`).then(async ({ html }) => {
                ...
                try {
                    ...
                    const achievements = []
                    const achievementrows = $('.table.table-striped.other-achievements').find('tr')
                    if (!achievementrows) return reject('Unloaded Graveyard')
                    for (var i = 1; i < achievementrows.length; i++) {
                        try {
                            const row = achievementrows[i]
                            achievements.push({
                    ...
                    // dungeon completes
                    const dungeonCompletes = []
                    const dungeonrows = $('.table.table-striped.main-achievements').find('tr')
                    if (!dungeonrows) return reject('Unloaded Graveyard')
                    for (var i = 1; i < dungeonrows.length; i++) {
                        try {
                            const row = dungeonrows[i]
        ...
        return new Promise(async (resolve, reject) => {
            handler.query(`https://www.realmeye.com/name-history-of-player/${ign}`).then(async ({ html }) => {
                ...
                try {
                    const pastNames = []
                    try {
                        const table = $('th.header').filter(function () { return $(this).text().trim() === 'Class' }).first().parent().parent().siblings('tbody').toArray()
            ...
            handler.query(`https://www.realmeye.com/fame-history-of-player/${ign}`).then(async ({ html }) => {
                ...
                try {
                    ...
                    for (const i of scripts) {
                        ...
                        const data = i.children[0].data.substring(i.children[0].data.indexOf('initializeGraphs') + 18, i.children[0].data.indexOf(',"f");') - 1)
                        data.split('],[').forEach(e => {
                            if (e.charAt(0) == '[') type++
                            const num = parseInt(e.split(',')[1].replace(/[^0-9]/gi, ''))
        ...
        return new Promise(async (resolve, reject) => {
            ...
            handler.query(`https://www.realmeye.com/exaltations-of/${ign}`).then(async ({ html }) => {
                ...
                for (var i = 0; i < rows.length; i++) {
                    try {
                        const row = rows[i]
                        const char = $(row).children('td:nth-child(2)').text()
                        characters[char] = {
        ...
        return new Promise(async (resolve, reject) => {
            handler.query(`https://www.realmeye.com/name-history-of-player/${ign}`).then(async ({ html }) => {
                ...
                try {
                    ...
                    for (const span of rows) {names.push($(span).text())}
                    resolve(names)
                } catch (error) {
    ...
    getSancProfile(ign) {
        return new Promise((resolve, reject) => {
            ...
            request.post('https://api.losthalls.org/getProfile', options, (err, res, body) => {
                if (!body.profile) {return reject(`No profile found for ${ign}.`)}
