...
var proxy = 'http://us-ca.proxymesh.com:31280'
var proxies = []
    ...
    return new Promise(res => {
        ...
        request(opt, (err, resp, html) => {
            ...
            for (let i of body.results) {
                p.push(`${i.proxy_address}:${i.ports.http}`)
            }
            res(p)
        })
    })
...
async function proxySwap() {
    ...
    proxy = proxies.splice(Math.floor(Math.random() * proxies.length), 1);

    //check proxy
    let r = await checkProxy(proxy)
    if (!r) proxySwap()
    else return
}
...
async function getOptions()
{
    ...
    const key = await new Promise((r) => pem.createCertificate({ days: 1, selfSigned: true }, (err, keys) => {
        if (err) return r();
        r(keys);
    }));
    if (key)
...
module.exports = {
    ...
    getUserInfo(ign, take2 = false, options) {
