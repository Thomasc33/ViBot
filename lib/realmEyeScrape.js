    ...
    return new Promise(res => {
        ...
        request(opt, async (err, resp, html) => {
            ...
            for (let i of body.results) {
                p.push(`https://${i.username}:${i.password}@${i.proxy_address}:${i.ports.http}`)
            }
            res(p)
        })
    })
...
module.exports = {
    ...
    getExaltationHistory(ign, take2 = false, options = null) {
        return new Promise(async (resolve, reject) => {
            if (!options) {
                options = await getOptions()
            }
            options = {
                url: `https://www.realmeye.com/exaltations-of/${ign}`,
                ...options
            }
            const search = ign;
            request(options, async function (err, resp, html) {
