...
Discord.MessageComponentInteraction.prototype.disableButton = async function disableButton(custom_id) {
    let newComponents = this.message.components.map(oldActionRow => {
        ...
        updatedActionRow.addComponents(oldActionRow.components.map(buttonComponent => {
          ...
          if(custom_id == buttonComponent.customId) newButton.setDisabled(true)
          return newButton
        }));
