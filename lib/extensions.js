 ...
 Discord.Message.prototype.confirmButton = function ConfirmYesNo(author_id) {
    ...
    return new Promise((resolve, reject) => {
        ...
        buttonCollector.on('end', async interaction => {
            const buttonsOff = new Discord.ActionRowBuilder().addComponents(buttonAccept.setDisabled(true), buttonDecline.setDisabled(true));
            this.edit({ components: [buttonsOff] }).catch(err => ErrorLogger.log(err, interaction.client));
            const err = 'Timed out.';
            err.stackTrace = new Error().stack;
...
const overrideDevs = ['277636691227836419', '258286481167220738', '120540036855889921', '178840516882989056']
