...
module.exports = {
    ...
    getRoleStrings(botSettings, member) {
        return member.roles.cache.map(role => Object.entries(botSettings.roles).filter(([, id]) => id == role.id).map(i => i[0])).flat()
    },
    createEmbed
}
...
async function createConfirmPanel(messageOrInteraction, text, image, confirmButton = new Discord.ButtonBuilder(), cancelButton = new Discord.ButtonBuilder(), messageType, time = MAX_WAIT, deletePanel = false) {
    ...
    async function deleteConfirmPanel(panel) {
        if (messageType == DiscordMessageType.MESSAGE_REPLY) panel.delete()
    }
...
Discord.GuildMember.prototype.can = function (role, bot) {
    if (typeof role == 'string') role = this.guild.roles.cache.get(this.client.settings[this.guild.id].roles[role])
    return bot.adminUsers.includes(this.id) || (role && this.roles.highest.comparePositionTo(role) >= 0)
}

Discord.GuildMember.prototype.canCommand = function (command, bot) {
    const settings = this.client.settings[this.guild.id]
    const role = this.guild.roles.cache.get(settings.roles[settings.commandRolePermissions[command.name] || command.role])
    return bot.adminUsers.includes(this.id) || (role && this.roles.highest.comparePositionTo(role) >= 0)
}

const supporterRoles = function (settings) {
    const { roles } = settings
    return [
        roles.supporterTierOne,
        roles.supporterTierTwo,
        roles.supporterTierThree,
        roles.supporterTierFour,
        roles.supporterTierFive,
        roles.supporterTierSix
    ]
}

Discord.GuildMember.prototype.supporterHierarchy = function (settings) {
    return 1 + supporterRoles(settings).map(s => this.roles.cache.has(s)).lastIndexOf(true)
}

Discord.Role.prototype.supporterHierarchy = function (settings) {
    return 1 + supporterRoles(settings).indexOf(this.id)
}

Discord.GuildMember.prototype.supporterRoleHierarchy = function (settings) {
    const hierarchy = this.supporterHierarchy(settings)
    if (!hierarchy) return null
    return supporterRoles(settings)[hierarchy - 1]
}
