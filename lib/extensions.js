...
Discord.GuildMember.prototype.supporterHierarchy = function supporterHierarchy(settings) {
    let roleId = this.roles.highest.id
    let roles = settings.roles
    let supporter = settings.supporter

    if (this.roles.cache.has(roles.supporterTierFive)) return 6
    else if (this.roles.cache.has(roles.supporterTierFour)) return 5
    else if (this.roles.cache.has(roles.supporterTierThree)) return 4
    else if (this.roles.cache.has(roles.supporterTierTwo)) return 3
    else if (this.roles.cache.has(roles.supporterTierOne)) return 2
    else if (this.roles.cache.has(roles.nitro)) return 1
    else return 0
}
