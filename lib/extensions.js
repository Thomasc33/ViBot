...
/**
 * Update this message with buttons ✅ ❌ and waits for the user to confirm. Returns true if ✅ was interacted and false if ❌ was interacted or the interaction times out.
 * @param {Discord.ButtonBuilder} confirmButton ButtonBuilder for confirm button.
 * @param {Discord.ButtonBuilder} cancelButton ButtonBuilder for cancel button.
 ...
 * @param {Number} time Time in milliseconds before interaction timeout. If not provided, will use MAX_WAIT.
 */
Discord.Message.prototype.confirmPanel = function ConfirmYesNo(confirmButton, cancelButton, authorID = false, time = MAX_WAIT) {
    confirmButton.setCustomId('Confirmed')
    cancelButton.setCustomId('Cancelled')
    const buttonActionRow = new Discord.ActionRowBuilder().addComponents([confirmButton, cancelButton])
    return new Promise(async (resolve) => {
        const buttonCollector = new Discord.InteractionCollector(this.client, { time: time, message: this, interactionType: Discord.InteractionType.MessageComponent, componentType: Discord.ComponentType.Button })
        await this.edit({ components: [buttonActionRow] })
        let resolved = false
        buttonCollector.on('collect', interaction => {
            if (!(authorID ? interaction.user.id == authorID : true)) return
            resolved = true
            buttonCollector.stop()
            resolve(interaction.customId === 'Confirmed')
        })
 ...
 * @param {Number} time Time in milliseconds before interaction timeout. If not provided, will use MAX_WAIT.
 */
Discord.ButtonInteraction.prototype.confirmPanel = function ConfirmYesNo(confirmButton, cancelButton, embed, reply = false, time = MAX_WAIT) {
    confirmButton.setCustomId('Confirmed')
    cancelButton.setCustomId('Cancelled')
    const buttonActionRow = new Discord.ActionRowBuilder().addComponents([confirmButton, cancelButton])
    return new Promise(async (resolve) => {
        if (reply) await this.reply({ content: null, embeds: [embed], ephemeral: true, components: [buttonActionRow]})
        else await this.editReply({ content: null, embeds: [embed], ephemeral: true, components: [buttonActionRow]})
        const message = await this.fetchReply()
        const buttonCollector = new Discord.InteractionCollector(this.client, { time: time, message: message, interactionType: Discord.InteractionType.MessageComponent, componentType: Discord.ComponentType.Button })
        let resolved = false
        buttonCollector.on('collect', interaction => {
            resolved = true
            buttonCollector.stop()
...
/**
 * Update this message with buttons ✅ ❌ and waits for the user to confirm. Returns true if ✅ was interacted and false if ❌ was interacted.
 * @param {Discord.StringSelectMenuBuilder} stringSelectMenu Discord Menu with options for the select menu provided.
 * @param {Discord.Snowflake?} authorID Id of the user we want to confirm with. If not provided, will accept any user's confirmation.
 * @param {Number} time Time in milliseconds before interaction timeout. If not provided, will use MAX_WAIT.
 */
Discord.Message.prototype.selectPanel = function ConfirmOne(stringSelectMenu, authorID = false, time = MAX_WAIT) {
    stringSelectMenu.addOptions({ label: 'Custom', value: 'custom' })
    const menuActionRow = new Discord.ActionRowBuilder().addComponents([stringSelectMenu])
    return new Promise(async (resolve) => {
        const menuCollector = new Discord.InteractionCollector(this.client, { time: time, message: this, interactionType: Discord.InteractionType.MessageComponent, componentType: Discord.ComponentType.StringSelect })
        const menuModalCollector = new Discord.InteractionCollector(this.client, { time: time, message: this, interactionType: Discord.InteractionType.ModalSubmit })
        await this.edit({ components: [menuActionRow] })
        let resolved = false
        menuCollector.on('collect', async interaction => {
            if (!(authorID ? interaction.user.id == authorID : true)) return
            if (interaction.values[0] === 'custom') {
                const menuModal = new Discord.ModalBuilder()
                    .setCustomId(`modal`)
                    .setTitle(`Afk Info`)
                const menuModalActionRow = new Discord.ActionRowBuilder().addComponents(
                    new Discord.TextInputBuilder()
                        .setCustomId(`modalField`)
                        .setLabel(`What would you like to input?`)
                        .setStyle(Discord.TextInputStyle.Short)
                        .setRequired(true)
                )
                menuModal.addComponents(menuModalActionRow)
                await interaction.showModal(menuModal)
            } else {
                resolved = true
                menuCollector.stop()
        ...
        menuCollector.on('end', interaction => {
            if (!resolved) resolve(null)
        });
        menuModalCollector.on('collect', async interaction => {
            if (!(authorID ? interaction.user.id == authorID : true)) return;
            resolved = true
            menuCollector.stop()
            menuModalCollector.stop()
 ...
 * @param {Number} time Time in milliseconds before interaction timeout. If not provided, will use MAX_WAIT.
 */
Discord.ButtonInteraction.prototype.selectPanel = function ConfirmOne(stringSelectMenu, embed, reply = false, time = MAX_WAIT) {
    stringSelectMenu.addOptions({ label: 'Custom', value: 'custom' })
    const menuActionRow = new Discord.ActionRowBuilder().addComponents([stringSelectMenu])
    return new Promise(async (resolve) => {
        if (reply) await this.reply({ content: null, embeds: [embed], ephemeral: true, components: [menuActionRow]})
        else await this.editReply({ content: null, embeds: [embed], ephemeral: true, components: [menuActionRow]})
        const message = await this.fetchReply()
        const menuCollector = new Discord.InteractionCollector(this.client, { time: time, message: message, interactionType: Discord.InteractionType.MessageComponent, componentType: Discord.ComponentType.StringSelect })
        const menuModalCollector = new Discord.InteractionCollector(this.client, { time: time, message: message, interactionType: Discord.InteractionType.ModalSubmit })
        let resolved = false
        menuCollector.on('collect', async interaction => {
            if (interaction.values[0] === 'custom') {
                const menuModal = new Discord.ModalBuilder()
 ...
 * @param {Number} time Time in milliseconds before interaction timeout. If not provided, will use MAX_WAIT.
 */
Discord.ButtonInteraction.prototype.confirmMenuPanel = function ConfirmYesNo(confirmButton, cancelButton, embed, reply = false, time = MAX_WAIT) {
    confirmButton.setCustomId('Confirmed')
    cancelButton.setCustomId('Cancelled')
    const buttonActionRow = new Discord.ActionRowBuilder().addComponents([confirmButton, cancelButton])
    return new Promise(async (resolve) => {
        ...
        const message = await this.fetchReply()
        const buttonCollector = new Discord.InteractionCollector(this.client, { time: time, message: message, interactionType: Discord.InteractionType.MessageComponent, componentType: Discord.ComponentType.Button })
        const buttonModalCollector = new Discord.InteractionCollector(this.client, { time: time, message: message, interactionType: Discord.InteractionType.ModalSubmit })
        let resolved = false
        buttonCollector.on('collect', async interaction => {
        ...
        buttonModalCollector.on('collect', async interaction => {
            resolved = true
            buttonCollector.stop()
            buttonModalCollector.stop()
            let modal = interaction.fields.getTextInputValue(`modalField`)
            await interaction.deferUpdate()
            resolve(modal)
        })
