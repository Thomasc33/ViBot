...
router.get('/afkchecks', (req, res) => {
    ...
    for (let i of Object.keys(afkChecks)) {
        for (let j of afkKeysToRemove) if (afkChecks[i][j]) delete afkChecks[i][j]
        if (afkChecks[i].runType && afkChecks[i].runType.location) delete afkChecks[i].runType.location
    }
