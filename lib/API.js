...
const afkKeysToRemove = new Set(['location', 'raidCommandsEmbed', 'raidCommandsMessage', 'raidInfoEmbed', 'raidInfoMessage', 'raidChannelsEmbed', 'raidChannelsMessage', 'message', 'guild', 'leader', 'buttons', 'earlyLocationMembers', 'earlySlotMembers', 'reactables', 'channel', 'pointlogMid'])
const axios = require('axios')
const { config: { botId, botSecret, meow }, settings }  = require('./settings');
const serverWhiteList = require('../data/serverWhiteList.json')
const crypto = require('crypto');
const algorithm = 'aes-256-gcm'; 
const secretKey = Buffer.from(meow, 'hex')
...
router.get('/afkchecks', (req, res) => {
    ...
    let afkChecksDto = Object.fromEntries(
        Object.entries(afkChecks).map(([key, value]) => {
            let newValue = Object.fromEntries(
                Object.entries(value).map(([k,v]) => {
                    if (k === 'members') return [k, v.length]
                    else if (k === 'deleted_by') return ['deleted', v !== null]
                    else if (k === 'ended_by') return ['ended', v !== null]
                    else if (k === 'aborted_by') return ['aborted', v !== null]
                    return [k,v]
                }).filter(([k]) => !afkKeysToRemove.has(k)))
            return [key, newValue]
        })
    )
    return res.status(200).json(afkChecksDto)
})
...
router.post('/currentweek/update', (req, res) => {
    ...
    if (!settings[guild.id].backend[currentweektype]) {
        res.status(403)
...
router.post('/auth', async (req, res) => {
    ...
    for (let server of serverWhiteList) {
        ...
        if (!member) continue
        const { roles } = settings[s.id]
        if (!roles) continue
        for (let r of adminRoles) {
            if (roles[r] && member.roles.cache.has(roles[r])) {
                isMod = true
