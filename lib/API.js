...
router.get('/user/nick/:uid/:guild', (req, res) => {
    ...
    const member = guild.members.cache.filter(user => user.nickname != null).find(nick => nick.nickname.replace(/[^a-z|]/gi, '').toLowerCase().split('|').includes(uid.toLowerCase()))
...
router.post('/members/:guild', (req, res) => {
    ...
    for (let i = 0; i < ids.length; i++) {
        ...
        const member = guild.members.cache.find(user => user.id.includes(ids[i]))
...
router.post('/currentweek/update', (req, res) => {
    ...
    let found = false
    for (const i of bot.guilds.cache.keys()) if (i == req.body.guildid) found = true
    if (!found) {
        res.status(400)
        return res.json(JSON.stringify('Bad guildid'))
    }
    const guild = bot.guilds.cache.get(req.body.guildid)
    const currentweektypes = ['currentweek', 'eventcurrentweek', 'parsecurrentweek']
    const index = req.body.currentweektype
    if (isNaN(parseInt(index)) || parseInt(index) >= currentweektypes.length) {
        res.status(400)
        return res.json(JSON.stringify('Bad currentweektype'))
    }
    const currentweektype = currentweektypes[parseInt(index)]
