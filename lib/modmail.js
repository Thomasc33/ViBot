...
const { settings: botSettings, config: { modmailGPTurl } } = require('./settings');
/**
 * @typedef ModmailData
...
async function performModmailReply(guild, attachments, content, raider, messageId) {
    ...
    const userModmailMessage = await directMessages?.messages.fetch(messageId).catch(() => {});
...
const Modmail = {
    ...
    async send({ settings, interaction, interaction: { guild, channel, message, member }, embed, raider }) {
        ...
        if (!performReply) return await message.edit({ components: getOpenModmailComponents(settings) });
    ...
    async forward({ settings, interaction, interaction: { guild, message, member }, embed, raider }) {
        ...
        if (!result) return await message.edit({ components: getOpenModmailComponents(settings) });
...
module.exports = {
    ...
    async sendModMail(message, guild, bot, db) {
        const settings = botSettings[guild.id];
