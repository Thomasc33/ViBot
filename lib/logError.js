    ...
    async log(error, bot) {
        ...
        if (error.name && error.message) {
            let embed = new Discord.MessageEmbed()
                .setColor('#ff0000')
                .setTitle('Error')
                .addField('Type', error.name.toString())
                .addField('Message', `\`\`\`${error.message.toString()}\`\`\``)
                .setTimestamp()
            if (error.method) embed.addField('Method', error.method.toString())
            if (error.path) embed.addField('Path', error.path.toString())
            if (error.code) embed.addField('Code', error.code.toString())
            if (error.httpStatus) embed.addField('httpStatus', error.httpStatus.toString())
